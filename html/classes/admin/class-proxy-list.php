 ...
 */
class RegRuDomains_Admin_Pages_ProxyList
{
	...
	public function __construct()
    {
        ...
        add_filter( 'manage_proxy_ip_posts_columns', array( __CLASS__, 'proxy_ip_posts_columns' ) );
        add_filter( 'manage_cars_posts_columns', array( __CLASS__, 'cars_posts_columns') );

        /**
         * @link https://awhitepixel.com/blog/modify-add-custom-columns-post-list-wordpress-admin/
         */
//        add_filter('manage_proxy_ip_posts_columns', function($columns) {
//            return array_merge($columns, ['verified' => __('Verified', 'regru_domains')]);
//        });
//
//        add_action('manage_proxy_ip_posts_custom_column', function($column_key, $post_id) {
//            if ($column_key == 'verified') {
//                $verified = get_post_meta($post_id, 'verified', true);
//                if ($verified) {
//                    echo '<span style="color:green;">'; _e('Yes', 'regru_domains'); echo '</span>';
//                } else {
//                    echo '<span style="color:red;">'; _e('No', 'regru_domains'); echo '</span>';
//                }
//            }
//        }, 10, 2);
//
...
//        add_action('restrict_manage_posts', array(__CLASS__, 'run_selected_campaigns'), 1, 2);
    ...
    public static function setup_post_type()
    {
        ...
        $labels = array(
            'name' => _x('proxy_ip', 'plural'),
            'singular_name' => _x('proxy_ip', 'singular'),
            'menu_name' => _x('proxy_ip', 'admin menu'),
            'name_admin_bar' => _x('proxy_ip', 'admin bar'),
            'add_new' => _x('Add New', 'add new'),
            'add_new_item' => __('Add New proxy_ip page'),
            'new_item' => __('New proxy_ip'),
            'edit_item' => __('Edit proxy_ip page'),
            'view_item' => __('View proxy_ip'),
            'all_items' => __('All proxy_ips'),
            'search_items' => __('Search proxy_ip'),
            'not_found' => __('No proxy_ip found.'),
        );
        $args = array(
            'supports' => $supports,
            'labels' => $labels,
        ...
        register_post_type( 'proxy_ip', $args );
    }


    public static function proxy_ip_posts_columns( $columns )
    {
        $columns['image'] = __( 'Image' );
        $columns['price'] = __( 'Price', 'regru_domains' );
        $columns['area'] = __( 'Area', 'regru_domains' );
        return $columns;
    }
    ...
    {
		add_submenu_page(
            $parent_slug  = 'regru_domains', __( 'REGRU DOMAINS Proxy List 1', 'regru_domains' ),
            $page_title   = esc_html__( 'REGRU DOMAINS Proxy List 2', 'regru_domains' ),
            $capability   = 'manage_options',
            $menu_slug    = self::$slug,
            $function     = array( self::class, 'html' )
        );
	}
     ...
     * @return array
     */
    public static function smashing_realestate_columns( $columns )
    {
        $columns = array(
            'cb' => $columns['cb'],
            'image' => __( 'Image' ),
            'title' => __( 'Title' ),
            'price' => __( 'Price', 'regru_domains' ),
            'area' => __( 'Area', 'regru_domains' ),
        );
        return $columns;
    }
...
//         * Proxy checker
//         */
//        $checker = new \App\Library\Proxy\Checker\ProxyChecker();
//        $proxy_check_result = $checker->check_array($proxies)->to_ip();
//
        /**
         * RegRu Filter
         */
        $regru = new \App\Library\RegRu\RegRu(new \App\Library\RegRu\Filters\RegRuDomainsFilter([
//            'filter_pr_from' => 0,
//            'filter_pr_to' => 800,
            'filter_withoutnumber' => 1,
            'filter_zone_ru' => 1,
            //'filter_dnames' => 'aaahh.ru',
        ]));
        $domain_list = $regru->download_from_csv($cnt = 5);
        dump(['dl' => $domain_list]);

        /**
         * Parser https://pr-cy.ru
         */
        $prcy_parser = new \App\Library\PRCY\PrCyParser();
        $prcy_parser->setProxies($proxies, false);
        ...
        foreach ($domain_list as $domain) {
            $data = $prcy_parser->check_backlinks($domain[0]);

            $result[$domain[0]] = $data;
