...
abstract class AbstractController implements ServiceSubscriberInterface
{
    ...
    protected function getParameter(string $name)
    {
        if (!$this->container->has('parameter_bag')) {
            throw new \Exception(sprintf('The "%s::getParameter()" method is missing a parameter bag to work properly. Did you forget to register your controller as a service subscriber? This can be fixed either by using autoconfiguration or by manually wiring a "parameter_bag" in the service locator passed to the controller.', static::class));
        }
...
//            'security.csrf.token_manager' => '?'.CsrfTokenManagerInterface::class,
     ...
     * @param IValidatorInterface $validator
     * @param bool $throwException
     * @return void
     * @throws \Exception
     */
    protected function validate(Request $request, IValidatorInterface $validator, bool $throwException = false)
    {
        $valid = $validator->validate($request);
        if ($throwException && ! $valid) {
            foreach ($validator->getErrors() as $error) {
                throw new \Exception(sprintf('Validation error for paramater %s with message %s', $error->get('path'), $error->get('message')));
            }
        }
        return $valid;
    }
